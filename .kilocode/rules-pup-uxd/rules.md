...
[本节集中定义适用于本页面的**所有通用业务逻辑**，包括权限、校验和操作后置逻辑，作为后续开发的主要依据。]
*   **权限规则 (Permissions):**
    *   **[元素/操作1 - '删除项目'按钮]:** 仅当登录用户角色为 `ADMIN` 或该项目的 `owner` 时可见并可操作。
    *   **[元素/操作2 - '归档'操作]:** 仅当项目状态 `status` 不为 `ARCHIVED` 时可用。
*   **校验规则 (Validations):**
    *   **[字段1 - '项目名称' (name)]:** 必填。提交时校验，长度不得超过50个字符。若为空或超长，在输入框下方显示错误提示：“项目名称为必填项，且不能超过50个字符。”
*   **状态流转规则 (State Transitions):**
    *   创建项目后，`status` 默认置为 `PENDING`。
    *   执行“归档”操作后，`status` 变为 `ARCHIVED`。

#### 3.4. 页面API契约 (Page API Contract)
...
### 4. 交互行为与用户流 (Interaction Behavior & User Flow)
本节内容的组织方式至关重要。你必须严格遵循以下原则，以确保交互逻辑的描述既严谨又易于理解：

1.  **核心组织单元：** 所有交互描述必须以**“用户场景”**为核心单元进行组织。
2.  **场景定义：** 每个用户场景都应是一个自包含的单元，需完整描述用户从**初始意图**到**任务最终完成（或失败）**的全部过程。
3.  **注意：** 通用的业务规则（如权限、校验等）已在 `3.3 核心业务规约` 中统一定义。本节聚焦于特定场景的独特交互流，并在必要时引用这些通用规则。

请基于 **`### 2. 页面原型设计`** 中的内容，并严格遵循如下的格式，构建若干核心任务的用户场景描述。
---
